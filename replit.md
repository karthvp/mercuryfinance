...
- **June 2026**: **Consolidated persistence into a single `app_data` JSONB column.** Every feature's data (balances, payrolls, expenses, incomes, debts, overrides, archived years) now lives in one column the app fully owns, with a one-time idempotent startup migration copying any data from the legacy per-feature columns. This stops new features from adding columns — which is what triggered Replit's destructive "DROP COLUMN" schema migrations on publish and caused data loss. The legacy columns are retained but unused (safe to drop once data is migrated).
